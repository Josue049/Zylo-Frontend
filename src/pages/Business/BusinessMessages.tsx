...
export default function BusinessMessagesPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      await markConversationAsRead(activeConvId, session.email, "business");
      const fresh = await getConversationMessages(activeConvId);
      setMessages(fresh);
  ...
  const handleSend = async () => {
    ...
    setInput("");
    setMessages(await getConversationMessages(activeConvId));
    await loadConversations();
  ...
  const totalUnread = conversations.reduce(
    (sum, c) => sum + c.businessUnreadCount,
    0,
  );
  ...
  return (
    <div className="bg-[#f9f6f5] h-screen font-body flex flex-col overflow-hidden">
      <HeaderBusiness />

      <div className="flex-1 max-w-5xl w-full mx-auto px-4 pt-24 pb-6 flex flex-col min-h-0 overflow-hidden">
        {/* Title */}
        ...
        <div className="flex-1 flex gap-4 min-h-0 overflow-hidden">
          {/* Conversation List */}
          <aside
            className={`flex flex-col gap-2 overflow-y-auto w-full md:w-80 md:min-w-[280px] md:flex shrink-0 ${isMobileChat ? "hidden md:flex" : "flex"}`}
          >
          ...
          {/* Chat Panel */}
          <section
            className={`flex-1 flex flex-col bg-white rounded-2xl shadow-sm overflow-hidden border border-[#e4e2e1] ${isMobileChat ? "flex" : "hidden md:flex"}`}
          >
            {!activeConv ? (
              <NoChatSelected />
            ) : (
              <>
                ...
                {/* Messages */}
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4 bg-[#f9f6f5] flex flex-col">
                  {messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center gap-3 text-[#5c5b5b]">
                      ...
                      <p className="text-sm max-w-xs">
                        El cliente aún no ha enviado mensajes. Puedes escribirle
                        primero.
                      </p>
                    </div>
                  ) : (
...
function ClientConversationItem({
  conv,
  isActive,
  onClick,
}: {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-3 p-4 rounded-2xl text-left transition-all active:scale-[0.98] ${isActive ? "bg-white shadow-md border border-[#ff785133]" : "bg-white hover:bg-[#f3f0ef] border border-transparent"}`}
    >
      {/* Avatar con inicial */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-center mb-0.5">
          <span
            className={`font-headline text-sm truncate ${unread > 0 ? "font-extrabold text-[#2f2f2e]" : "font-bold text-[#2f2f2e]"}`}
          >
            {conv.userName}
          </span>
        ...
        <span
          className={`text-xs truncate block ${unread > 0 ? "text-[#2f2f2e] font-medium" : "text-[#5c5b5b]"}`}
        >
  ...
  return (
    <div className={`flex w-full ${isOwn ? "justify-end" : "justify-start"}`}>
      ...
      >
        ...
        <div
          className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed break-words ${isOwn ? "bg-gradient-to-br from-[#ab2d00] to-[#ff7851] text-white rounded-br-sm" : "bg-white text-[#2f2f2e] shadow-sm border border-[#e4e2e1] rounded-bl-sm"}`}
        >
          {msg.text}
        </div>
    ...
    <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center px-8">
      ...
      <div>
        ...
        <p className="text-sm text-[#5c5b5b] max-w-xs">
          Selecciona una conversación para ver los mensajes de tus clientes y
          responderles.
        </p>
      </div>
    </div>
    ...
    <div className="flex flex-col items-center justify-center gap-4 py-16 text-center px-4">
      ...
      <div>
        ...
        <p className="text-sm text-[#5c5b5b]">
          Cuando un cliente te escriba desde tu perfil, aparecerá aquí.
        </p>
      </div>
    </div>
  );
