          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            <section className="bg-white p-8 rounded-xl shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {TEAM.map((m) => (
                  <div key={m.name} className="flex items-center gap-4 p-4 border border-[#afadac]/10 rounded-2xl hover:bg-[#f3f0ef] transition-all">
                    <img className="w-12 h-12 rounded-full object-cover" src={m.avatar} alt={m.name} />
                    <div>
                      <p className="font-bold text-[#2f2f2e]">{m.name}</p>
                      <p className={`text-xs font-semibold ${m.roleColor}`}>{m.role}</p>
                    </div>
                    <button className="ml-auto material-symbols-outlined text-[#5c5b5b] text-lg hover:text-[#2f2f2e] transition-colors">more_vert</button>
                  </div>
                ))}
              </div>
            </section>
